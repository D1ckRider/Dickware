...
static bool ComboFilter__DrawPopup(int& state, int START, const char **ENTRIES, int ENTRY_COUNT)
{
...
void MenuHelper::components::ComboFilterBox(std::string label, const char * items[], int size, int & value)
{
	...
	using namespace ImGui;
	char* SearchString = "input text here";
	bool done = InputText(label.data(), SearchString, IM_ARRAYSIZE(SearchString), ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_EnterReturnsTrue);
	bool hot = value >= 0 && strcmp(SearchString, items[value]);
	if (hot) 
	{
		int new_idx = fuzzy::search(SearchString, size, items);
		int idx = new_idx >= 0 ? new_idx : value;
		//s.selectionChanged = s.activeIdx != idx;
		value = idx;
		if (done || ComboFilter__DrawPopup(value, idx, items, size)) {
			int i = value;
			if (i >= 0) {
				strcpy(SearchString, items[i]);
				done = true;
			}
		}
	}
...
void MenuHelper::components::Hotkey(std::string label, int & value)
{
	ImGui::Hotkey(label.data(), &value);
}
